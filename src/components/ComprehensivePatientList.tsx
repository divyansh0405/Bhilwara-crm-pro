...
const ComprehensivePatientList: React.FC<ComprehensivePatientListProps> = ({ onNavigate }) => {
  ...
  const loadPatients = async () => {
    try {
      ...
      // This includes:
      // 1. Patients with is_confirmed === false (from external bookings like appointments-app)
      // 2. Patients with pending appointments in localStorage
      patientsData = patientsData.filter(patient => {
        // Check if patient is unconfirmed from external booking (appointments-app)
        // These patients should appear in Dashboard's Upcoming Appointments first
        if (patient.is_confirmed === false) {
          console.log(`👤 Hiding patient ${patient.first_name} ${patient.last_name} - UNCONFIRMED from external booking`);
          return false; // Hide this patient - they should appear in dashboard appointments first
        }

        // Also check localStorage appointments for this patient
        try {
        ...
        return true; // Show this patient (confirmed and no pending appointments)
      });
  ...
  const handleShiftToIPD = (patient: PatientWithRelations) => {
    // IPD functionality is currently disabled
    toast.error('IPD Beds functionality is currently disabled');
    return;

    // DISABLED: IPD Beds tab is hidden
        ...
        <div className="flex justify-between items-start mb-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3 py-2 text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all duration-200 flex items-center gap-2"
            >
              🔄 Refresh Data
            </button>
            <button
              ...
              className="px-3 py-2 text-sm bg-green-600 text-white rounded-lg hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
      ...
      {filteredPatients.length > 0 ? (
        ...
        <div className="bg-white rounded-xl shadow-lg border p-16 text-center">
          ...
          <button
            ...
            className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 font-medium transition-all duration-200 shadow-md hover:shadow-lg"
          >
            🔄 Refresh List
          </button>
        </div>
